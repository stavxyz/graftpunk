...
Redirect chains are recorded one HAR entry per hop, linked by `response.redirectURL`, so a login form whose `POST` answers with a `302` shows up as the `POST` (with its redacted body and the `Set-Cookie` of the redirect) followed by the request it redirected to.
