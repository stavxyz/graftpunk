...
Both backends also subscribe to `Network.requestWillBeSentExtraInfo` / `responseReceivedExtraInfo`, the events that carry the raw wire headers Chromium filters out of `responseReceived`: the HAR's `response.headers` include `Set-Cookie` (also parsed into `response.cookies`) and `request.headers` include the raw `Cookie` header (parsed into `request.cookies`). On the login `POST → 302` hop that means the cookie the server minted is visible on the hop that minted it. One measured caveat: nodriver 0.48.1 delivers only a fraction of `requestWillBeSentExtraInfo` events (1 of 11 on a test login, none carrying `Cookie`), so raw request `Cookie` headers effectively need nodriver ≥ 0.50.1 (9 of 11 delivered, 6 with `Cookie`); `Set-Cookie` capture works on both.
