...
Redirect chains are recorded one HAR entry per hop, linked by `response.redirectURL`, so a login form whose `POST` answers with a `302` shows up as the `POST` (with its redacted body, status and `Location`) followed by the request it redirected to.

**One current gap:** `Set-Cookie` does not appear in any entry's response headers: Chromium delivers only filtered headers on `Network.responseReceived`, and the raw ones arrive via `responseReceivedExtraInfo`, which the capture does not subscribe to yet ([#157](https://github.com/stavxyz/graftpunk/issues/157)).
