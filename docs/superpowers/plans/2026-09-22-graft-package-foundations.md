...
- Produces: `CommandMetadata.endpoint: str | None = None`; `command(..., endpoint: str | None = None)`; `pysrc.import_lines(module: str, *names: str) -> list[str]`; every stub rendered from the digest carries `endpoint="<METHOD> <template>"` as a decorator keyword starting its own line (a long value wraps as an implicit concatenation, which reads back through `ast` as one string), the no-digest placeholder stub declares none, and a stub with an `int`, `float`, `bool`, or list parameter carries `params=[PluginParamSpec.option(...), ...]` for every parameter, built by `_param_spec(identifier, keywords, *, click_kwargs=())`: a `bool` one as a flag with a negative (`click_kwargs={"is_flag": True, "flag": "--name/--no-name"}`, or `--name/--name-false` when `--no-name` is another option, or the positive-only `--name` with a `GP-FILL` comment when both are taken, from `_negatable_flag(identifier, option_names)`; the handler gets `True`, `False`, or `None`), a list one as `click_kwargs={"multiple": True}`. Identifiers are seeded with `self`, `ctx`, and `_RESERVED_OPTION_IDENTIFIERS` (the CLI's built-in options and `help`). `_declared_extras(endpoint)` returns `dict[str, _Declaration]` (label, annotation, keywords, multiple) keyed by the site name, or `body_<name>` for a body field typed apart from its query namesake (`_body_field_keys` maps each sent body field to its key), and `_undeclared_body_fields(endpoint)` the JSON body fields no option can send, which the stub names in a `GP-FILL` comment. The digest types a query or form value `int`, `float`, or `bool` only when the typed value is sent back spelled as recorded (`bool` is lowercase `true`/`false`), a query or form parameter whose requests disagree is `str`, and a JSON field keeps its JSON type (`int` with `float` is `float`, other disagreement `mixed`); `object`, `mixed`, and `list[<element>]` complete the labels. The project-tools plan's reader reads the `endpoint` keyword from source, and its `render_command` reuses `_declared_extras` and `_needs_param_specs` as they are and `_decorator_lines` with a changed signature: that plan's Task 5 makes it `_decorator_lines(command: PlannedCommand, param_specs: list[str]) -> list[str]`, so the help placeholder, the name pin, and the endpoint all come from the planned command.
