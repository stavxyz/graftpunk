...
- Location 9 (decided at PR review, 2026-08-25): a Rich panel *title* is hard-truncated into the top border with no ellipsis, so the 119-character subtitle was cut mid-word at every terminal width under ~137 columns. The panel title is the literal `graftpunk`; the subtitle is the first body line under the version, where Rich wraps it.
