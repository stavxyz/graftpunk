...
type: spec
validated:
  sha: 4caaa05f0f0ba6287500c38ade601d6469f93bb8
  date: 2026-09-01T20:08:28Z
...
3. **Resolution precedence** `--session` flag > `GRAFTPUNK_SESSION` env > `.gp-session` in cwd, then: exactly one cached session for the plugin → use it; several → refuse with a typed error listing candidates. Never "most recent wins". (Flag > env > per-directory file matches the common CLI convention — verified for git by probe; the AWS CLI/pyenv analogy is unverified — and, decisively, graftpunk's existing order: `src/graftpunk/session_context.py:64` (`def resolve_session`), `src/graftpunk/session_context.py:14` (`def get_active_session`).)
...
- **The charset is a backend contract, enforced by a test:** the `SessionStorageBackend` protocol docstring (`src/graftpunk/storage/base.py:135` (`class SessionStorageBackend`)) states the session-name charset (including the single `@`), and a shared, parametrized backend-conformance test round-trips a labelled `name@label` through each backend's serializer — so a fourth backend inherits a failing test, not a reading assignment.
...
- `derive_account_identity(fields: Mapping[str, str], secret_keywords: frozenset[str]) -> tuple[str | None, str | None]` — the **pure derivation policy** (identifier pick via the keyword-major scan, slugified label). It lives here, not in the CLI: identifier and label derivation is naming policy, this module's charter. `make_login_body` stays the orchestrating call site that owns only the CLI-specific parts (the `--as` option, the warning emission, the stamp).
...
Every entry point computes the **operating session name** exactly once, and the *chain itself has one home*: `session_identity.compute_operating_session_name(explicit, base, existing_names, *, use_ambient=True)` composes `--session` flag > `GRAFTPUNK_SESSION` > `.gp-session` (delegating the ambient tiers to the existing `src/graftpunk/session_context.py:64` (`def resolve_session`)) > `resolve_account_session`. Each surface is a one-line call with its surface-specific inputs; a surface that deliberately skips the ambient tiers says so via the named parameter, never by re-implementing the chain. That one string is the key for **both the load and every write-back** of that invocation. `plugin.session_name` is demoted, explicitly, to *base name: an input to resolution*, never a storage key at command time.

Where it lives per surface:

- **Plugin commands** (`gp <site> <cmd>` — the surface where the 2026-08-24 collision actually happened): today this path bypasses all selection — `src/graftpunk/cli/plugin_runtime.py:79` (`session = plugin.get_session() if needs_session else requests.Session()`) loads via the plugin's own `get_session` (`src/graftpunk/plugins/cli_plugin.py:1150` (`return load_session_for_api(self.session_name)`)), which reads the ambient `self.session_name`, and plugin commands have no `--session` option (`src/graftpunk/cli/command_factory.py:144` (`BUILTIN_OPTIONS: dict[str, Any] = {"format": "json", "view": (), "output": ""}`)). The design: `session` joins `BUILTIN_OPTIONS`, so every synthesized plugin command accepts `--session <name>`; the runtime computes the operating name at command entry, loads via `load_session_for_api(operating_name)` (`src/graftpunk/cache.py:539` (`def load_session_for_api(name: str) -> requests.Session:`)) instead of `plugin.get_session()`, carries it in `CommandContext` (the field exists: `src/graftpunk/cli/plugin_runtime.py:163` (`_session_name=(plugin.session_name if needs_session else "")`)), and keys the post-command write-back with it (`src/graftpunk/cli/plugin_runtime.py:207` (`update_session_cookies(session, plugin.session_name)`) changes to the operating name).
- **Python API**: `GraftpunkClient` (`src/graftpunk/client.py:200` (`class GraftpunkClient`)) gains `session: str | None = None`. `__init__` computes the operating name once via `compute_operating_session_name(session, base, names, use_ambient=False)` — **a deliberate decision**: library code is not steered by ambient shell state (`GRAFTPUNK_SESSION`, `.gp-session`), so the client's chain is pin > account resolution, expressed as a named mode of the shared composition function rather than a hand-rolled shorter chain — stores it, and **both** load and the write-backs at `src/graftpunk/client.py:402` (`update_session_cookies(session, plugin.session_name)`) and `src/graftpunk/client.py:431` (`update_session_cookies(self._session, self._plugin.session_name)`) use it. One resolution implementation shared with the CLI; `AmbiguousSessionError` propagates to the caller. **Superseded by #181 (2026-09-04):** the client resolves on first session use, not in `__init__` — construction performs no session-storage I/O and never raises for session state; the operating name is computed at the lazy load in `_execute_command`, and both write-backs still key off it.
...
> **Design note (2026-08-26):** `GP_ACCOUNT_ATTR` is the third fact riding the session object (after `_gp_cached_tokens` and `_gp_header_roles`). If a fourth appears, consolidate the riders into one declared registry (a tuple of attribute names the serialization whitelist, the extractor and the login funnel all consume) so the pickle coupling is visible in one place.

**The identifier rides the session object**, following the existing idiom for session-derived metadata (`session_name`, `current_url`): the login path sets the attribute named by `GP_ACCOUNT_ATTR` once, `_extract_session_metadata` (`src/graftpunk/cache.py:177` (`def _extract_session_metadata`)) reads it via `getattr` like the fields it already collects, and — because `BrowserSession.__getstate__`/`__setstate__` (`src/graftpunk/session.py:554` (`def __getstate__`) / `src/graftpunk/session.py:619` (`def __setstate__`), **both** backend paths) are extended to carry the `GP_ACCOUNT_ATTR` attribute, exactly as they already do for `_gp_cached_tokens` and `_gp_header_roles` — the attribute survives load/save round-trips. (Pickling is whitelist-based, both in `BrowserSession.__getstate__` and in `requests.Session.__attrs__`; without this extension the first `update_session_cookies` refresh would silently erase the recorded identifier. Whether `GraftpunkSession` — a plain `requests.Session` subclass with no custom pickle methods — must also carry the attribute for the `load_session_for_api_from_bytes` round-trip is a plan-time decision.) `cache_session`'s signature does **not** change and it stays **read-free**: every existing writer, most importantly the refresh path `update_session_cookies` (`src/graftpunk/cache.py:641` (`def update_session_cookies`)), preserves the identity because it re-caches the same (unpickled) object, which carries the whitelisted attribute.

### Login flow

- `gp <site> login` gains `--as <label>` (validated: same charset as the label half of a session name).
- **One owner for derivation — the CLI login wrapper.** `make_login_body` (`src/graftpunk/cli/login_commands.py:143` (`def make_login_body`)) is where the resolved credentials dict and the secret policy (`src/graftpunk/cli/login_commands.py:32` (`SECRET_KEYWORDS = frozenset`)) already live, and it wraps **both** the declarative path and hand-written `login(credentials)` methods (credentials resolved at `src/graftpunk/cli/login_commands.py:181` (`credentials: dict[str, str] = {}`)). It calls `session_identity.derive_account_identity` (the pure policy — see that section) with the resolved fields and the secret policy: iterate the identifier keywords `username`, `email`, `login`, `identifier`, `user` in that order (keyword-major — for each keyword, scan fields in declaration order; the `user`⊂`username` overlap is intentional because `username` is checked first), taking the first field whose name contains the keyword; if none matches, the first non-secret field. The **label** is the slugified identifier; `--as` overrides the label but not the recorded identifier.
...
- **Scope of the stamp — mechanical, not documentary:** the wrapper applies the stamp through a context manager that restores the prior state (deletes the instance shadows) when the login flow exits, so "meaningful only until the login's cache write completes" is enforced by code, and a plugin instance held across a login cannot leak the stamped identity into later reads. Longer-lived hosts (keepalive, embedded clients) read the operating session name from the resolution owner, never from the plugin instance. When the login engine is next restructured, thread the name and identifier explicitly through `_cache_login_session`'s signature and retire the stamp.
...
`AmbiguousSessionError(GraftpunkError)` joins `src/graftpunk/exceptions.py:83` (`class TokenExtractionError`)'s file, carrying the base name and the candidate session names. **One shared renderer** — a small function beside the `gp_console` helpers — prints the candidates (with lazily fetched account identifiers) plus the three ways to pick (`--session`, `GRAFTPUNK_SESSION`, `gp session use`). Every CLI boundary that can see the error calls that renderer: the plugin-runtime error boundary, the `session_commands` call sites, and the `observe` callback — the rendering exists once, whichever door the error exits through. The renderer's docstring states the obligation ("every CLI entry point that can surface `AmbiguousSessionError` calls this"), so a future entry point discovers the rule at the one place it would look; if the CLI later grows a single top-level `GraftpunkError` boundary, the renderer registers there once and the per-boundary wiring retires. It is **not** a `ValueError`: nothing raised `ValueError` on this path before, so there is no compatibility to preserve.
...
- Identity survives a load → `update_session_cookies` re-cache round-trip — exercised on both the nodriver and selenium `__getstate__`/`__setstate__` paths, which must be extended to carry `GP_ACCOUNT_ATTR` (pickling is whitelist-based; whole-`__dict__` survival cannot be assumed).
- `SessionMetadata` round-trip with and without `account_identifier` on all three backends' serializers; pre-upgrade metadata (field absent) still loads; the parametrized backend-conformance test round-trips a labelled `name@label` through each backend.
