...
- **Observe HAR now carries `Set-Cookie` and the raw `Cookie` header** ([#157](https://github.com/stavxyz/graftpunk/issues/157)). Chromium delivers only filtered headers on `Network.responseReceived`; both capture backends now subscribe to the `*ExtraInfo` events and merge the raw wire headers into each entry, parsed into HAR `response.cookies` / `request.cookies`. On an observed login the `POST → 302` hop shows the session cookie it minted. Raw request `Cookie` headers depend on nodriver delivering `requestWillBeSentExtraInfo`, which 0.48.1 mostly does not (measured 1 of 11 events) and 0.50.x does (9 of 11); `Set-Cookie` works on both.
