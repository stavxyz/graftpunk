...
- **`GraftpunkClient` resolves its session on first use, not at construction** ([#181](https://github.com/stavxyz/graftpunk/issues/181)) — construction never performs storage I/O or raises for session reasons; `AmbiguousSessionError` (several accounts cached, unpinned) and `SessionNotFoundError` surface from the first `execute()` — catch `AmbiguousSessionError` next to `SessionNotFoundError`. Lazy resolution makes the error *catchable*, not handled: a wrapper that only names `SessionNotFoundError` still lets ambiguity through, but now from inside the call it already wraps, so the fix is one `except` clause rather than restructuring construction. "Pick one" is also the right instruction — logging in again would add a third slot, not resolve the ambiguity. Pin with `GraftpunkClient("myshop", session="myshop@alice")` to skip resolution (and its listing) entirely.
