...
- `gp session clear <base>`, `gp session show <base>`, `gp session use <base>`, and `gp http --session <base>` operate on the exact cached slot when one exists under that name, and only resolve accounts when it does not. Previously a registered site name was always resolved, so with a legacy bare slot next to a labelled one the documented `gp session clear <base>` step exited with the ambiguity list and removed nothing.
...
5. **Stale bare-name sessions after the first labelled login.** The first `gp <site> login` after upgrading writes a new `base@label` slot and leaves the legacy bare `base` slot in place. From that point, an unpinned command refuses with `AmbiguousSessionError` naming both candidates, and library code calling `load_session_for_api("base")` keeps loading the stale bare slot, because an exact name match wins over resolution. Upgrading alone does not touch cached sessions; this first appears on the first re-login after upgrading. Before removing the stale slot, fix any code that still calls the low-level `load_session("base")` (importable as `graftpunk.load_session` or `graftpunk.cache.load_session`): that function is literal and raises `SessionNotFoundError` once the bare slot is cleared and only the labelled slot remains. Use `load_session_for_api` (or `load_session_for_api_resolved` when writing the session back), or pin the labelled name. The `load_session_for_api` and `load_session_for_api_resolved` swap fits callers that only need the cookies and headers carried on the returned `requests.Session`. A caller that needs the cached session object itself should pin the labelled name. Then the remedy is a one-time manual step: run `gp session clear base` once after that first login to remove the stale slot, or pin the session explicitly with `--session base@label`, `GRAFTPUNK_SESSION`, or `session=` in code.
