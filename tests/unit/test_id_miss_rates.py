...
(``field_names_regression.py``), the tuned corpus of public SDK and API names
the name rule's thresholds were set against (``field_names_tuned.py``), and a
fresh corpus of field, header, and cookie names written after those thresholds and
not tuned against (``field_names_fresh.py``). A response map is also keyed by ids when 3 or
...
ceiling (2026-09-23): lower36_8 0.99975, lower36_12 0.998, upper36_10
...
from tests.unit.field_names_tuned import TUNED_NAMES
...
# the path rule last changed (2026-09-23): lower36_8 0.08175, lower36_12 0.01625,
# upper36_10 0.036, token_urlsafe_16 0.026, nanoid_21 0.03. Each ceiling is the
# rate measured when the name rule was simplified, plus 0.001 (four tokens).
_SHAPES = {
...
# The share of each corpus read as ids, measured 2026-09-23: 0 of 482 regression
# names, 0 of 259 tuned names, and 0 of 229 fresh names. The tuned ceiling is the
# 1% the name rule was set to meet; the regression ceiling is the one it had before
# the name rule was simplified; the fresh ceiling is its measured rate plus one
# name.
...
_CORPORA = [
    ...
    ("tuned", TUNED_NAMES, 200, _TUNED_CEILING),
